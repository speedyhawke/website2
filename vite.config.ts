    ...
    try {
      ...
      if (fs.existsSync(indexPath)) {
        ...
        if (fs.existsSync(distAssetsDir)) {
          // Create unhashed fallback copies so any direct /assets/index.js request also gets latest code
          ...
          if (jsBundle) {
            fs.copyFileSync(path.join(distAssetsDir, jsBundle), path.join(distAssetsDir, 'index.js'));
          }
          if (cssBundle) {
            fs.copyFileSync(path.join(distAssetsDir, cssBundle), path.join(distAssetsDir, 'index.css'));
          }
