...
export const StJohnsHeroHeader: React.FC<StJohnsHeroHeaderProps> = ({
  ...
  const [heroImage, setHeroImage] = useState<string>(() => {
    ...
    // If it's a previous placeholder unsplash url or legacy /scenic_nl.jpg, clear and use the authentic image
    if (custom && (custom.includes('unsplash.com') || custom === '/scenic_nl.jpg' || custom === 'scenic_nl.jpg')) {
                    ...
                    >
                      <div className="w-full h-14 rounded-lg overflow-hidden relative mb-1.5 bg-slate-950">
                        <img
                          ...
                          onError={(e) => {
                            const target = e.currentTarget;
                            if (preset.id === 'fillthegap_st_johns_battery') {
                              if (!target.dataset.fallbackStep) {
                                target.dataset.fallbackStep = '1';
                                target.src = './scenic_nl.jpg';
                              } else if (target.dataset.fallbackStep === '1') {
